...
<div>
    <div class="relative z-0 w-full mb-6 group">
      ...
      <select id="id_categoria" name="id_categoria" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-500 focus:border-primary-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500">
        @foreach ($categorias as $categoria)
          <option value="{{ $categoria->id }}" {{ isset($categoria) && $categoria->id_categoria == $categoria->id ? "selected" : "" }}>{{ $categoria->nombre }}&nbsp;{{ $categoria->descripcion }}</option> 
        @endforeach  
        {{-- selected="" --}}
      </select>
    </div>
    <div class="relative z-0 w-full mb-6 group">
      <input value="{{ $producto->uri_foto ?? ''}}" type="text" id="uri_foto" name="uri_foto" class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer" placeholder=" " required/>
      <label for="uri_foto" class="peer-focus:font-medium absolute text-sm text-gray-500 dark:text-gray-400 duration-300 transform -translate-y-6 scale-75 top-3 -z-10 origin-[0] peer-focus:left-0 peer-focus:text-blue-600  peer-focus:dark:text-blue-500 peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-75 peer-focus:-translate-y-6">Url Del Producto</label>
    </div>
    <div class="relative z-0 w-full mb-6 group">
    ...
    <div class="relative z-0 w-full mb-6 group">
      <input value="{{ $producto->titulo ?? '' }}" type="text" name="titulo" id="titulo" class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer" placeholder=" " required />
    ...
    <div class="relative z-0 w-full mb-6 group">
      ...
      <select id="id_estado" name="id_estado" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-500 focus:border-primary-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500">
        @foreach ($estados as $estado)
          <option value="{{ $estado->id }}" {{ isset($producto) && $producto->id_estado == $estado->id ? "selected" : "" }}>{{ $estado->nombre }}&nbsp;{{ $estado->descripcion }}</option> 
        @endforeach
      </select>
    </div>
    <div class="relative z-0 w-full mb-6 group">
