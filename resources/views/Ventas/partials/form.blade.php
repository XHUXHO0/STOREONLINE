...
<div>
    <div class="relative z-0 w-full mb-6 group">
      <label for="id_vendedor" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Vendedor</label>
      <select id="id_vendedor" name="id_vendedor" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-500 focus:border-primary-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500">
        @foreach ($vendedores as $vendedor)
          <option value="{{ $vendedor->id }}" {{ isset($producto) && $producto->id_vendedor == $vendedor->id ? "selected" : "" }}>{{ $vendedor->nombre }}&nbsp;{{ $vendedor->descripcion }}</option> 
        @endforeach  
      </select>
    </div>
    <div class="mb-6">
